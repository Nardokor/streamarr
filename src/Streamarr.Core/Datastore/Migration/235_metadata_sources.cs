            ...
            Execute.Sql(@"
                ...
                SELECT
                    ...
                    json_object(
                        ...
                        'defaultDownloadVideos',        COALESCE(CAST((SELECT Value FROM Config WHERE Key='YouTubeDefaultDownloadVideos') AS INTEGER), 1),
                        'defaultDownloadShorts',        COALESCE(CAST((SELECT Value FROM Config WHERE Key='YouTubeDefaultDownloadShorts') AS INTEGER), 1),
                        'defaultDownloadVods',          COALESCE(CAST((SELECT Value FROM Config WHERE Key='YouTubeDefaultDownloadVods') AS INTEGER), 1),
                        'defaultDownloadLive',          COALESCE(CAST((SELECT Value FROM Config WHERE Key='YouTubeDefaultDownloadLive') AS INTEGER), 0),
                        ...
                        'defaultWatchedDefeatsIgnored', COALESCE(CAST((SELECT Value FROM Config WHERE Key='YouTubeDefaultWatchedDefeatsIgnored') AS INTEGER), 1),
                        'defaultAutoDownload',          COALESCE(CAST((SELECT Value FROM Config WHERE Key='YouTubeDefaultAutoDownload') AS INTEGER), 1)
                    ),
                    1,
                    '[]',
                    1
